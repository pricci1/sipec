...
const AnimalMoves = () => {
  ...
  async function getEstablishment() {
    ...
    setestablishment(
      Establecimento.data.map(({ name, rup }) => ({
        value: rup,
        label: rup + "/" + name
      }))
    );
    ...
    async function getAnimalMoves(
      ...
      if (destinyRUP != "") {
        movesMap = movesMap.filter(d => d.rup_destination == destinyRUP);
      }
                ...
                <div className={"col-md-11"}>
                  <div className={"col-md-3"}>
                    ...
                    <Autocomplete
                      ...
                      renderItem={(item, isHighlighted) => (
                        <div className="form-control">{item.label}</div>
                      )}
                      value={values.originRUP}
                      onChange={autocompleteOriginRUP}
                      onSelect={val => setFieldValue("originRUP", val)}
                    />
                  </div>
                  <div className={"col-md-3"} style={{ marginBottom: 10 }}>
                    ...
                    <Autocomplete
                      ...
                      value={values.destinyRUP}
                      onChange={autocompleteDestinyRUP}
                      onSelect={val => setFieldValue("destinyRUP", val)}
                    />
                  </div>
                </div>
                <div>
